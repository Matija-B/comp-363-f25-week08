...
def sim():
    ...
    queue_samples = 0
    sum_queue = 0
   #This helps us keep the simulation going 
   #Until we reach the total_ticks
    timer = 0
    while timer < TOTAL_TICKS:
        for (i, j) in list(in_transit.keys()):
            # We move the cars to the queue if we arrive 
            pop_to_queue_if_arrived(i, j)
            #Based on the traffic light condition we want to serve each intersectio0n
        for node in nodes:
            serve_intersection(timer, node)

        for (i, j) in links:
            #We want to check if there is a boundry link coming 
            if is_boundary_incoming_link(i, j):
                if random.random() < ARRIVAL_RATE:
                    #We then create a new car 
                    newCar = Car(id = car_id, t_enter = timer)
                    #We then check our enqueue_depature.
                    if enqueue_departure(i, j, newCar):
                        #If it is enqueued then we want to add 1 in car_id
                        car_id += 1

        #We want to get our total queue.
        total = sum(len(q) for q in stopped.values())
        sum_queue = sum_queue + total
    ...
    print("Time taken: ", TOTAL_TICKS)
    print("Traffic lights cycle: NS green ", CYCLE_NS_GREEN, " EW green ", CYCLE_EW_GREEN)
    print("Arrival rate at boundary: ", ARRIVAL_RATE)
    print("Number Trips Completed: ", completed)
    print("Throughput: ", completed/TOTAL_TICKS)
    print("Average travel time in grid", sum_tt/completed)
    print("Average number of queue vehicles: ", sum_queue/max(1, queue_samples))
